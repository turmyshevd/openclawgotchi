...
### Changed
- **RAG transports split into two independent URLs.** REST and MCP-SSE deployments of `rag-core` run side-by-side (REST on `:8765`, MCP-SSE gateway on `:8766`); the bot now mirrors that. New env vars `RAG_REST_URL` and `RAG_MCP_URL`, set one / the other / both. Legacy `RAG_API_URL` + `RAG_TRANSPORT=rest|mcp` still honored via a compat shim in `config.py` — existing deployments don't need to change anything. Removes the prior limitation where REST and MCP couldn't coexist (they shared `RAG_API_URL`).

### Added
- **Time-of-day awareness + user-editable quiet schedule.** New `gotchi-skills/time/` skill, new `src/utils/timing.py` with the canonical `dd_mm_yyyy-hh_mm_ss` timestamp helper and a span-based quiet schedule (`data/quiet_schedule.json`, gitignored). New `/quiet` Telegram command lets the user paint 24h with verbosity spans (silent/quiet/normal/chatty) — no SSH needed. Heartbeat skips when verbosity is `silent`, but critical auto-mood states (OOM, OVERHEATING) still hit the display. `setup.sh` now runs `timedatectl set-timezone Europe/Berlin` (override via `OCG_TIMEZONE=…`) and enables NTP sync at install time.
- **External RAG service integration via REST**: opt-in connector to any RAG (Retrieval-Augmented Generation) backend that exposes a small documented HTTP contract (see `src/llm/rag_client.py` module docstring). New `/rag` Telegram command (`/rag`, `/rag <query>`, `/rag --top N <query>`). LLM tools `query_rag(query, top_k)` and `persist_to_rag(text, title, tags)`. Env vars: `RAG_REST_URL` (empty disables), `RAG_API_KEY`, `RAG_DEFAULT_COLLECTIONS`.
...
- **Two new LLM tools for MCP**: `mcp_list_tools()` (discover) and `mcp_call_tool(name, arguments)` (invoke). The agent decides which advertised tool to call. Activates when `RAG_MCP_URL` is set in the environment.
